...
def attente_clic():
    """
    Attend que l'utilisateur clique sur la fenêtre et renvoie un triplet (x,
    y, type_clic), où x et y sont l'abscisse et l'ordonnée du point cliqué,
    et type_clic une chaîne valant 'ClicGauche' ou 'ClicDroit' selon le type
    de clic effectué.
    """
    while True:
        ev = donne_evenement()
...
def attente_clic_ou_touche():
    ...
    La fonction renvoie un couple (type, val), où type vaut 'Touche',
    'ClicGauche' ou 'ClicDroit' selon le type d'événement reçu, et 'val' est
    soit :
    - un couple (x, y) indiquant les coordonnées du clic si l'événement est
    un clic de souris ;
    - une valeur correspondant à la touche pressée sinon.

    :return: un couple (type, val) où val est soit le couple de coordonnées
    du clic, soit la valeur de la touche.
    """
    while True:
        ev = donne_evenement()
        type_ev = type_evenement(ev)
        if type_ev == "ClicDroit" or type_ev == "ClicGauche":
            return type_ev, (clic_x(ev), clic_y(ev))
        elif type_ev == "Touche":
            return type_ev, touche(ev)
        mise_a_jour()
